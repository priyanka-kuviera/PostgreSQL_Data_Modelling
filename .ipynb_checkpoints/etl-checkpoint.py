...
def process_song_file(cur, filepath):
    ...
    # insert song record
    song_data = df.get(['song_id','title','artist_id','year','duration']).values[0].tolist()
    cur.execute(song_table_insert, song_data)
    
    # insert artist record
    artist_data = df.get(['artist_id','artist_name','artist_location','artist_latitude','artist_longitude']).values[0].tolist()
